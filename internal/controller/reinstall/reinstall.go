...
func (r *ReinstallHandler) ProcessRequest(
	...
	if reinstallRequestProcessedCondition.Reason == string(v1alpha1.Initialized) {
		...
		if err = patchClusterInstanceReinstallStatus(ctx, r.Client, log, clusterInstance, patch); err != nil {
			log.Sugar().Errorf("Failed to update reinstall condition '%s'", reinstallRequestProcessedCondition.Type,
				zap.Error(err))
			return reconcile.Result{}, err
		}
		log.Sugar().Infof("Updated reinstall condition '%s' successfully", reinstallRequestProcessedCondition.Type)
	...
	defer func() {
		...
		if changed || updateEndtime {
			updateErr := patchClusterInstanceReinstallStatus(ctx, r.Client, log, clusterInstance, patch)
			if updateErr != nil {
				if err == nil {
					err = updateErr
				}
...
func (r *ReinstallHandler) ensureValidReinstallRequest(
	...
	if changed := setReinstallStatusCondition(clusterInstance, *reinstallRequestCondition); changed {
		updateRequired = true
	}
	...
	if updateRequired {
		...
		updateErr := patchClusterInstanceReinstallStatus(ctx, r.Client, log, clusterInstance, patch)
		if updateErr != nil {
			errMsg := "failed to update reinstall request validation status"
			log.Error(errMsg, zap.Error(updateErr))
			if err == nil {
				err = updateErr
			} else {
				err = fmt.Errorf("%s (%w), err: %w", errMsg, updateErr, err)
			}
		}
	}
...
func (r *ReinstallHandler) ensureRenderedManifestsAreDeleted(ctx context.Context, log *zap.Logger,
	...
	excludeFromDeletion := make([]ci.RenderedObject, 1)
	...
	if !deletionCompleted {
		...
		result.RequeueAfter = deletionRequeueWithMediumInterval
		return
	}
...
func (r *ReinstallHandler) finalizeReinstallRequest(
	...
	if updateRequired {
		if err := patchClusterInstanceReinstallStatus(ctx, r.Client, log, clusterInstance, patch); err != nil {
			log.Error("Failed to update completed reinstall request status")
			return err
		}
	}
	return nil
