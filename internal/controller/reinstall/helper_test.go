...
var _ = Describe("Reinstall Condition Status Functions", func() {
	...
	type conditionTestCase struct {
		function     func(metav1.ConditionStatus, v1alpha1.ClusterInstanceConditionReason, string) *metav1.Condition
		typeExpected v1alpha1.ClusterInstanceConditionType
	}

	var (
		testStatus  metav1.ConditionStatus
		testReason  v1alpha1.ClusterInstanceConditionReason
		testMessage string
	)
	...
	testCases := []conditionTestCase{
		{reinstallRequestProcessedConditionStatus, v1alpha1.ReinstallRequestProcessed},
		{reinstallRequestValidatedConditionStatus, v1alpha1.ReinstallRequestValidated},
		{reinstallPreservationDataBackedupConditionStatus, v1alpha1.ReinstallPreservationDataBackedup},
		{reinstallClusterIdentityDataDetectedConditionStatus, v1alpha1.ReinstallClusterIdentityDataDetected},
		{reinstallRenderedManifestsDeletedConditionStatus, v1alpha1.ReinstallRenderedManifestsDeleted},
		{reinstallPreservationDataRestoredConditionStatus, v1alpha1.ReinstallPreservationDataRestored},
	}

	for _, tc := range testCases {
		func(tc conditionTestCase) {
			It("should return a correct condition for "+string(tc.typeExpected), func() {
				condition := tc.function(testStatus, testReason, testMessage)

				Expect(condition).NotTo(BeNil())
				Expect(condition.Type).To(Equal(string(tc.typeExpected)))
...
var _ = Describe("patchClusterInstanceReinstallStatus", func() {
	...
	It("should return an error if patching fails", func() {
		...
		err := patchClusterInstanceReinstallStatus(ctx, c, testLogger, clusterInstance, client.MergeFrom(clusterInstance.DeepCopy()))
